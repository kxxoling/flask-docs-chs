...
.. autoclass:: Request
   ...
   .. attribute:: form
      ...
      包含解析过的从 `POST` 或 `PUT` 请求发送的表单对象的
      :class:`~werkzeug.datastructures.MultiDict` 对象。
      请注意：上传的文件不在这里，而是在 :attr:`files` 属性中。

   .. attribute:: args

      包含解析过的查询字符串（ URL 中问号后的部分）内容的
      :class:`~werkzeug.datastructures.MultiDict` 对象。

   .. attribute:: values

      包含 :attr:`form` 和 :attr:`args` 全部内容的
      :class:`~werkzeug.datastructures.CombinedMultiDict` 对象。

   .. attribute:: cookies

      包含请求中传送的所有 cookie 内容的 :class:`dict` 对象。

   .. attribute:: stream

      如果表单提交的数据没有以已知的 mimetype 编码，出于性能考虑，数据会不经
      修改存储在该 stream 中。大多数情况下，使用字符串格式的
      :attr:`data` 会是更好的方法。stream 只返回一次数据。
   ...
   .. attribute:: data
      ...
      如果进入的请求数据是 Flask 不能处理的 mimetype ，数据将以字符串形式存储在这里。

   .. attribute:: files

      一个包含 `POST` 和 `PUT` 请求中上传的文件的 
      :class:`~werkzeug.datastructures.MultiDict` 对象。每个文件都是一个 
      :class:`~werkzeug.datastructures.FileStorage` 对象。其操作行为类似 Python 
      的标准文件对象，不同之处在于该对象提供一个
   ...
   .. attribute:: url_root
      ...
      提供不同的方式来审视当前的 URL 。比如你的应用监听了下面的 URL::
   ...
   .. attribute:: is_xhr
      ...
      所有支持 ``X-Requested-With`` 头并把头设置为 `XMLHttpRequest` 的库奏效，
      其中包括 prototype 、 jQuery 以及 Mochikit 等等。
...
只需要设置了 :attr:`Flask.secret_key` ，你就可以在 Flask 应用中使用会话。
会话使得在请求见保留信息成为可能。 Flask 的实现方法是使用一个签名的 cookie ，
这样用户虽然可以查看会话的内容，但是无法对其进行修改，除非用户获取了密钥。
因此需要确保密钥是一个复杂、无法被轻易猜测的值。

你可以使用 :class:`session` 对象来访问当前的会话:

.. class:: session

   会话对象很像普通的字典，区别是会话对象会追踪修改。
...
关于更多的用法，请阅读 :mod:`json` 文档。下面的拓展已经默认被集成
到了标准库中 JSON 模块里：
